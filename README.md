...
```bash
docker run -itv `pwd`:`pwd` -e HOME=`pwd`  --platform=linux/amd64  keng404/nextflow-to-icav2-config:0.0.5 /bin/bash
```

where ````pwd```` is your ```$HOME``` directory. As you are running this image as ```root``` you may want to change ownership of files generated by the commands below back to the ```id``` and ``gid``` of your user on your desktop/server.
