...
parser$add_argument("-s", "--nf-script","--nf_script", default=NULL,
                    help="Main NF script")
parser$add_argument("-i","--other-workflow-scripts","--other_workflow_scripts", default=c(""),nargs="?",
...
if(length(other_workflow_scripts) > 0 | !is.null(nf_script)){
  ...
  if(length(other_workflow_scripts)>0){
    for(i in 1:length(other_workflow_scripts)){
      add_error_stub(nf_file=other_workflow_scripts[i],error_stub=error_stub_to_add)
    }
  }
  if(!is.null(nf_script)){
    add_error_stub(nf_file=nf_script,error_stub=error_stub_to_add) 
  }
}
